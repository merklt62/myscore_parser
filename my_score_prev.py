...
def write():
    ...
    writer = pd.ExcelWriter('prev_day_table.xlsx', engine='xlsxwriter')
...
def write_xlsx():
    ...
    df = pd.DataFrame(data_1)
    writer = pd.ExcelWriter('all_table_prev_day.xlsx', engine='xlsxwriter')
    df.to_excel(writer, sheet_name='Sheet1', index=False)
    writer.save()
